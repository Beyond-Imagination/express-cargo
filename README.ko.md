...
| `@IsHash(algorithm: HashAlgorithm, message?: string)`                         | 필드가 주어진 해시 알고리즘에 대한 유효한 해시 문자열인지 검증합니다. 지원: `md5`, `sha1`, `sha256`, `sha384`, `sha512`, `crc32`, `crc32b`. | `@IsHash('sha256') checksum!: string`                                                      |
