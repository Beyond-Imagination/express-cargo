...
| `@ArrayMaxSize(max: number, message?)`                                      | 배열의 요소 수가 `max` 이하이어야 함                                                                        | `@ArrayMaxSize(5) tags!: string[]`                                                         |
...
| `@IsUrl(options?: IsUrlOptions, message?: string)`                          | 필드가 유효한 URL인지 검증합니다. 기본적으로 `http`와 `https` 프로토콜만 허용하며, `options.protocols`로 변경할 수 있습니다. | `@IsUrl() website!: string`                                                                |
