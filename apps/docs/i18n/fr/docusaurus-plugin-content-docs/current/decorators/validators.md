...
Valide que le champ décoré est un nombre hexadécimal. Seuls les caractères `0-9` et `a-f` (insensible à la casse) sont autorisés. Les préfixes `0x` et `0h` sont également autorisés.
