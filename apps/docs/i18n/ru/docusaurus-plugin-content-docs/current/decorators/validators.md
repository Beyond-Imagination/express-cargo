...
`@Enum` тоже проверяет значение своего поля, но относится к категории типов-помощников, а не к валидаторам ниже. Он описан в разделе [Декораторы типов-помощников](./type-helpers.md), и эта разница важна, когда вы берётесь за `@Each`.
...
Проверяет, что декорированное поле является допустимым идентификатором часового пояса IANA (например, `Asia/Seoul`, `America/New_York`, `UTC`). Использует встроенный API `Intl` — внешние зависимости не требуются.
...
`@Each` оборачивает только декораторы валидации. Передача декоратора источника, типа-помощника, отсутствующего значения или преобразования отклоняется при регистрации маршрута — например, `@Each(Enum(UserRole))` падает с ошибкой `@Each cannot wrap type-helper decorator(s): @Enum`.
...
Ошибки валидации выбрасываются как `CargoValidationError` — как показано в шаге 4 примера. Типы ошибок, рекомендуемый путь через `setCargoErrorHandler` и примеры ответов см. в [Обработка ошибок](../examples/validation-errors.md).
