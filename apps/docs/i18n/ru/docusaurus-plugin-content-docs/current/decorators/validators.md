...
Проверяет, что декорированное поле является допустимым URL. По умолчанию разрешены только протоколы `http` и `https`.

- **`options`** (необязательно):
  - **`protocols`**: Массив разрешённых протоколов. По умолчанию: `['http', 'https']`.
- **`message`** (необязательно): Сообщение об ошибке, которое будет отображаться при сбое валидации. Если опущено, будет использоваться сообщение по умолчанию.
