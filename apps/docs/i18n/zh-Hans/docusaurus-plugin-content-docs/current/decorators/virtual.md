...
`@Virtual` 和 `@Request` 属于两个不同的分类，一个字段只能带其中一个。同时使用二者会在注册路由时报出 `@Request cannot be combined with @Virtual`，而把其中任意一个与 `@Body` 这样的来源装饰器搭配也会被同样拒绝——三者回答的是同一个问题：字段的值从哪里来。

- 当值位于原始 `Request` 对象上，且需要在对象自身字段绑定之前取得时，使用 `@Request`。
- 当值由正在构建的对象的其他字段推导得出时，使用 `@Virtual`。

`@Virtual` 在所有来源字段绑定完成之后执行，因此无论这些字段以什么顺序声明都能读取到。只有依赖另一个 `@Virtual` 字段的 `@Virtual` 字段才对顺序敏感。
