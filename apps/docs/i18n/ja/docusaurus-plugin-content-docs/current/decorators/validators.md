...
`@Enum` もフィールドの値を検証しますが、以下のバリデータではなく型ヘルパーのカテゴリに属します。[型ヘルパーデコレータ](./type-helpers.md) で説明しており、この違いは `@Each` を使うときに効いてきます。
...
`@Each` が包めるのはバリデーションデコレータだけです。ソース・型ヘルパー・値なし処理・変換のデコレータを渡すとルート登録時に拒否されます。たとえば `@Each(Enum(UserRole))` は `@Each cannot wrap type-helper decorator(s): @Enum` で失敗します。
...
バリデーションの失敗は、例の手順 4 のとおり `CargoValidationError` として投げられます。エラーの型、推奨される `setCargoErrorHandler` の方法、レスポンス例については [エラーハンドリング](../examples/validation-errors.md) を参照してください。
