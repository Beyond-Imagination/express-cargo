...
**A:** **`@Optional()`** デコレータを使用します。値が `null` または `undefined` でもフィールドは正常にバインドされ、そのフィールドのバリデーションはスキップされます。代わりにフォールバック値を割り当てるには、`@Default()` を使用します。[欠落フィールドの処理](./decorators/missing-fields.md)を参照してください。
</details>
