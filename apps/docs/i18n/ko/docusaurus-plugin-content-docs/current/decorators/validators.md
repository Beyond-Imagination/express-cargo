...
데코레이터가 적용된 속성이 영문 알파벳 문자(A–Z / a–z)와 숫자(0–9)만 포함하고 있는지 확인합니다.
...
데코레이터가 적용된 속성이 유효한 UUID인지 확인합니다.

- **`version`** (선택 사항): 허용할 UUID 버전을 지정합니다. 지정한 경우 해당 버전의 UUID만 유효한 값으로 판단하며, 생략하면 모든 UUID 버전을 허용합니다.
  (예: `4` → UUID v4만 허용)
- **`message`** (선택 사항): 검증 실패 시 표시할 메시지. 생략하면 기본 메시지가 사용됩니다.
