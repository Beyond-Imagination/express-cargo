...
| `@IsHexadecimal(message?: string)`                                         | Validates that the field is a hexadecimal number (characters `0-9` and `a-f`, case-insensitive). The `0x` and `0h` prefixes are also allowed.                   | `@IsHexadecimal() color!: string`                                                                                  |
